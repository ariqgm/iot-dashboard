...
const db = mysql.createConnection({
  ...
  database: 'testing_arq' // Ganti dengan nama database Anda
});
...
app.get('/api/data15menit', (req, res) => {
  const query = 'SELECT * FROM status_beban_daya_listrik WHERE timestamp >= NOW() - INTERVAL 15 MINUTE ORDER BY timestamp ASC';
...
// Konfigurasi MQTT
const mqttClient = mqtt.connect('mqtt://192.168.1.14:1883', {
  username: 'ariq',
  password: '1234'
});
...
mqttClient.on('connect', function () {
  ...
  const topics = ['Status_Daya_Beban_Listrik/#', 'Proteksi_Listrik', 'home/status'];
...
// Inisialisasi status lampu kamar
...
mqttClient.on('message', function (topic, message) {
  ...
  try {
    ...
    } else if (topic === 'Proteksi_Listrik') {
...
io.on('connection', (socket) => {
  ...
  // Endpoint untuk mengambil data 15 menit terakhir
